...
export default function Home() {
	...
	// Update progress timeline berdasarkan scrollY
	useEffect(() => {
		if (timelineRef.current) {
			const progress = timelineList[section];
	...
	useEffect(() => {
        ...
        function isScrollable(element: HTMLElement | null) {
            return element?.closest(".scrollable") !== null;
        }

		function handleTouchStart(event: TouchEvent) {
            if (isScrollable(event.target as HTMLElement)) return;
        ...
        function nextSection() {
            if (section + 1 < timelineList.length) {
        ...
        function handleTouchEnd(event: TouchEvent) {
            ...
            // Pastikan jarak swipe cukup besar sebelum dianggap sebagai scroll
            if (Math.abs(startY - endY) > SCROLL_THRESHOLD) {
                event.preventDefault();
		...
		window.addEventListener("scroll", doNothing, { passive: false });
		window.addEventListener("keydown", doNothing, { passive: false });
		...
		return () => {
			...
			window.removeEventListener("scroll", doNothing);
			window.removeEventListener("keydown", doNothing);
		};
	}, [section, timelineList]);
